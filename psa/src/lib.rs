...
pub trait ModuleAnalyzer {
    ...
    fn detect_content_root(&self, module_path: &str) -> ContentRoot {
        ...
        match self.detect_source_root(module_path) {
            Some(source_root) => content_root.add_source_root(source_root.as_str()),
            _ => (),
        }
        match self.detect_resource_root(module_path) {
            Some(resource_root) => content_root.add_resource_root(resource_root.as_str()),
            _ => (),
        }
        match self.detect_test_source_root(module_path) {
            Some(test_source_root) => content_root.add_test_source_root(test_source_root.as_str()),
            _ => (),
        }
        match self.detect_test_resource_root(module_path) {
